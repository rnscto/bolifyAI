...
import { Upload, Loader2, FileText, CheckCircle2, AlertCircle, ArrowRight, ArrowLeft, FileSpreadsheet, X } from 'lucide-react';
...
export default function CSVImportDialog({ open, onOpenChange, clientId, onComplete }) {
    ...
    LEAD_FIELDS.forEach(field => {
      const match = detectedHeaders.find(h => {
        ...
        return hLower === fLower || hLower === fLabel ||
          ...
          hLower.includes(fLabel) || fLabel.includes(hLower) ||
  ...
  const getValidLeads = () => {
    return getMappedLeads().filter(lead => {
      // At least one of phone, name, or email must be present
      return (lead.phone && lead.phone.length > 0) ||
  ...
  const handleImport = async () => {
    ...
    if (validLeads.length === 0) {
      toast.error('No valid leads to import. At least name, phone, or email is needed.');
      return;
    }
    ...
    setResult({
      ...
      skipped: rawData.length - imported,
    });
    setStep(4);
    setImporting(false);
  ...
  const fileTypeIcon = file?.name?.endsWith('.csv') ? '📄' : '📊';
          ...
          <div className="space-y-4">
            ...
            <div className="bg-blue-50 rounded-lg p-4">
              ...
              <ul className="text-xs text-blue-700 space-y-1">
                ...
                <li>• Rows with no identifiable data will be skipped</li>
              </ul>
            </div>
          </div>
        )}
                    ...
                    >
                      ...
                      <SelectContent>
                        <SelectItem value="_skip">
                          <span className="text-gray-400">— Skip / Don't import —</span>
                        </SelectItem>
                        {fileHeaders.map(h => (
          ...
          <div className="space-y-4">
            ...
            {getSkippedLeads().length > 0 && (
              <div className="flex items-start gap-2 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                ...
                <p className="text-xs text-yellow-700">
                  {getSkippedLeads().length} row(s) have no name, phone, or email and will be skipped.
                </p>
              </div>
            )}
              ...
              <Table>
                ...
                <TableBody>
                  {getMappedLeads().slice(0, 20).map((lead, i) => {
                    const isValid = (lead.phone && lead.phone.length > 0) ||
                                    (lead.name && lead.name.length > 0) ||
                                    (lead.email && lead.email.length > 0);
                    return (
            ...
            <div className="flex gap-3 justify-between">
              ...
              <Button
                ...
                disabled={importing || getValidLeads().length === 0}
                className="bg-blue-600 hover:bg-blue-700"
              >
                {importing ? (
                  <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Importing...</>
                ) : (
                  <><Upload className="w-4 h-4 mr-2" /> Import {getValidLeads().length} Leads</>
                )}
              </Button>
            </div>
          </div>
        ...
        {step === 4 && result && (
          <div className="text-center py-6 space-y-4">
            ...
            <div>
              ...
              {result.skipped > 0 && (
                <p className="text-sm text-gray-500 mt-1">{result.skipped} rows skipped (no identifiable data)</p>
              )}
            </div>
