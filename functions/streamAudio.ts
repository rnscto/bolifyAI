                  ...
                  content: `Analyze this call transcript:\n\n${transcript}\n\nReturn JSON:
{
  ...
  "recommended_next_action": "..."
    ...
    // ===== STEP 5.5: ADMIN ALERT for hot/interested leads =====
    // This runs directly via Resend (no Base44 integration credits needed)
    if (qualificationTier === 'hot' || leadStatus === 'interested' || leadStatus === 'converted') {
      try {
        const { Resend } = await import('npm:resend@4.0.0');
        const resendClient = new Resend(Deno.env.get('RESEND_API_KEY'));
        
        // Get client info for context
        const clientInfo = currentLog.client_id ? 
          await serviceClient.entities.Client.get(currentLog.client_id).catch(() => null) : null;
        
        // Get admin emails: client owner + platform admin
        const adminEmails = ['sales@vaaniai.io'];
        if (clientInfo?.email) adminEmails.push(clientInfo.email);
        
        const leadName = currentLog.lead_id ? 
          (await serviceClient.entities.Lead.get(currentLog.lead_id).catch(() => null))?.name || 'Unknown' : 'Unknown';
        const leadPhone = currentLog.callee_number || currentLog.caller_id || '';
        const leadEmail = currentLog.lead_id ? 
          (await serviceClient.entities.Lead.get(currentLog.lead_id).catch(() => null))?.email || '' : '';

        const alertHtml = `
<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
  <div style="background:linear-gradient(135deg,#dc2626,#b91c1c);padding:20px 24px;border-radius:12px 12px 0 0;">
    <h2 style="color:white;margin:0;">🔥 HOT LEAD ALERT — Immediate Action Required</h2>
  </div>
  <div style="padding:24px;background:white;border:1px solid #e5e7eb;border-top:none;">
    <table style="width:100%;border-collapse:collapse;">
      <tr><td style="padding:8px 0;color:#6b7280;width:120px;">Lead Name:</td><td style="padding:8px 0;font-weight:bold;">${leadName}</td></tr>
      <tr><td style="padding:8px 0;color:#6b7280;">Phone:</td><td style="padding:8px 0;font-weight:bold;"><a href="tel:${leadPhone}">${leadPhone}</a></td></tr>
      ${leadEmail ? `<tr><td style="padding:8px 0;color:#6b7280;">Email:</td><td style="padding:8px 0;"><a href="mailto:${leadEmail}">${leadEmail}</a></td></tr>` : ''}
      <tr><td style="padding:8px 0;color:#6b7280;">Score:</td><td style="padding:8px 0;font-weight:bold;color:#dc2626;">${leadScore}/100 — ${qualificationTier.toUpperCase()}</td></tr>
      <tr><td style="padding:8px 0;color:#6b7280;">Status:</td><td style="padding:8px 0;">${leadStatus}</td></tr>
      <tr><td style="padding:8px 0;color:#6b7280;">Sentiment:</td><td style="padding:8px 0;">${sentiment}</td></tr>
      <tr><td style="padding:8px 0;color:#6b7280;">Duration:</td><td style="padding:8px 0;">${Math.floor(duration/60)}m ${duration%60}s</td></tr>
      ${clientInfo ? `<tr><td style="padding:8px 0;color:#6b7280;">Client:</td><td style="padding:8px 0;">${clientInfo.company_name}</td></tr>` : ''}
    </table>
    <div style="margin-top:16px;padding:16px;background:#fef2f2;border-radius:8px;border-left:4px solid #dc2626;">
      <p style="margin:0;font-weight:bold;color:#991b1b;">Call Summary:</p>
      <p style="margin:8px 0 0 0;color:#374151;">${summary.substring(0, 500)}</p>
    </div>
    ${intentSignals.length > 0 ? `<p style="margin-top:12px;color:#6b7280;font-size:13px;">Intent Signals: ${intentSignals.join(', ')}</p>` : ''}
    <div style="margin-top:20px;text-align:center;">
      <a href="tel:${leadPhone}" style="display:inline-block;background:#dc2626;color:white;padding:12px 32px;border-radius:8px;text-decoration:none;font-weight:bold;">📞 Call Lead Now</a>
    </div>
  </div>
  <div style="padding:12px 24px;background:#f9fafb;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px;text-align:center;">
    <p style="margin:0;color:#9ca3af;font-size:12px;">VaaniAI Automation Engine — ${new Date().toLocaleString('en-IN', {timeZone:'Asia/Kolkata'})}</p>
  </div>
</div>`;

        for (const email of [...new Set(adminEmails)]) {
          resendClient.emails.send({
            from: 'VaaniAI Alerts <noreply@vaaniai.io>',
            to: email,
            subject: `🔥 HOT LEAD: ${leadName} (${leadPhone}) — Score ${leadScore}/100 — CONNECT NOW`,
            html: alertHtml
          }).then(() => console.log(`[${reqId}] 📧 Admin alert sent to ${email}`))
            .catch(e => console.error(`[${reqId}] ⚠️ Admin alert failed for ${email}: ${e.message}`));
        }
      } catch (alertErr) {
        console.error(`[${reqId}] ⚠️ Admin alert error: ${alertErr.message}`);
      }
    }
