import { Hono } from "hono";
import { jwt } from "hono/jwt";
import { client } from "../db/index.ts";

export const entityRouter = new Hono();

const JWT_SECRET = Deno.env.get("JWT_SECRET") || "super_secret_bolifyai_key";

// Secure all entity routes with JWT authentication
entityRouter.use("*", jwt({ secret: JWT_SECRET, alg: "HS256" }));

// Utility to sanitize table names to prevent SQL injection
const sanitizeTableName = (name: string) => {
  return name.toLowerCase().replace(/[^a-z0-9_]/g, "");
};

// Cache for table columns to prevent constant querying
const tableColumnsCache = new Map<string, Set<string>>();

async function getValidColumns(entity: string): Promise<Set<string>> {
  if (tableColumnsCache.has(entity)) {
    return tableColumnsCache.get(entity)!;
  }
  const result = await client.queryObject(
    `SELECT column_name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = $1`,
    [entity]
  );
  const columns = new Set<string>(result.rows.map((row: any) => row.column_name));
  tableColumnsCache.set(entity, columns);
  return columns;
}

// GET /api/entities/:entity - list/filter
entityRouter.get("/:entity", async (c) => {
  const entity = sanitizeTableName(c.req.param("entity"));
  const queryParams = c.req.query();
  const user = c.get("jwtPayload") as any;
  
  const validCols = await getValidColumns(entity);
  if (validCols.size === 0) {
    return c.json({ error: "Entity not found" }, 404);
  }

  let query = `SELECT * FROM "${entity}"`;
  const conditions: string[] = [];
  const args: any[] = [];
  let paramIndex = 1;

  // MULTI-TENANCY ENFORCEMENT
  if (validCols.has("client_id")) {
    conditions.push(`"client_id" = $${paramIndex}`);
    args.push(user.client_id);
    paramIndex++;
  } else if (entity === "client") {
    // If querying the client table, user can only query their own client
    conditions.push(`"id" = $${paramIndex}`);
    args.push(user.client_id);
    paramIndex++;
  }

  for (const [key, value] of Object.entries(queryParams)) {
    if (key === 'limit' || key === 'offset' || key === 'sort') continue;
    if (!validCols.has(key)) {
      // Filtering by a column that doesn't exist means no rows can possibly match
      return c.json([]);
    }
    conditions.push(`"${key}" = $${paramIndex}`);
    args.push(value);
    paramIndex++;
  }

  if (conditions.length > 0) {
    query += ` WHERE ${conditions.join(" AND ")}`;
  }

  // Handle sort if provided
  if (queryParams.sort) {
    const sortField = queryParams.sort.startsWith('-') ? queryParams.sort.substring(1) : queryParams.sort;
    const sortOrder = queryParams.sort.startsWith('-') ? 'DESC' : 'ASC';
    if (validCols.has(sortField)) {
      query += ` ORDER BY "${sortField}" ${sortOrder}`;
    }
  }

  // Handle limit and offset
  if (queryParams.limit) {
    query += ` LIMIT ${parseInt(queryParams.limit) || 100}`;
  } else {
    query += ` LIMIT 100`; // Default limit
  }

  try {
    const result = await client.queryObject(query, args);
    return c.json(result.rows);
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
});

// GET /api/entities/:entity/:id - get one
entityRouter.get("/:entity/:id", async (c) => {
  const entity = sanitizeTableName(c.req.param("entity"));
  const id = c.req.param("id");
  const user = c.get("jwtPayload") as any;

  const validCols = await getValidColumns(entity);

  try {
    let query = `SELECT * FROM "${entity}" WHERE id = $1`;
    const args: any[] = [id];

    // MULTI-TENANCY ENFORCEMENT
    if (validCols.has("client_id")) {
      query += ` AND client_id = $2`;
      args.push(user.client_id);
    } else if (entity === "client") {
      query += ` AND id = $2`;
      args.push(user.client_id);
    }

    query += ` LIMIT 1`;

    const result = await client.queryObject(query, args);
    if (result.rows.length === 0) {
      return c.json({ error: "Not found" }, 404);
    }
    return c.json(result.rows[0]);
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
});

// POST /api/entities/:entity - create
entityRouter.post("/:entity", async (c) => {
  const entity = sanitizeTableName(c.req.param("entity"));
  const body = await c.req.json();
  const user = c.get("jwtPayload") as any;
  
  const validCols = await getValidColumns(entity);
  if (validCols.size === 0) {
    return c.json({ error: "Entity not found" }, 404);
  }

  // Filter body to only include valid columns and exclude auto-generated ones
  const filteredBody: Record<string, any> = {};
  for (const [k, v] of Object.entries(body)) {
    if (validCols.has(k) && k !== 'id' && k !== 'created_at' && k !== 'updated_at') {
      filteredBody[k] = v;
    }
  }

  // MULTI-TENANCY ENFORCEMENT: Force inject client_id
  if (validCols.has("client_id")) {
    filteredBody["client_id"] = user.client_id;
  }

  const columns = Object.keys(filteredBody).map(k => `"${k}"`).join(", ");
  const placeholders = Object.keys(filteredBody).map((_, i) => `$${i + 1}`).join(", ");
  const values = Object.values(filteredBody);

  let query = `INSERT INTO "${entity}" DEFAULT VALUES RETURNING *`;
  if (columns.length > 0) {
     query = `INSERT INTO "${entity}" (${columns}) VALUES (${placeholders}) RETURNING *`;
  }

  try {
    const result = await client.queryObject(query, values);
    return c.json(result.rows[0], 201);
  } catch (error: any) {
    console.error("POST Error:", error);
    return c.json({ error: error.message }, 500);
  }
});

// PUT /api/entities/:entity/:id - update
entityRouter.put("/:entity/:id", async (c) => {
  const entity = sanitizeTableName(c.req.param("entity"));
  const id = c.req.param("id");
  const body = await c.req.json();
  const user = c.get("jwtPayload") as any;
  
  console.log(`[PUT /api/entities/${entity}/${id}] Received payload:`, JSON.stringify(body).slice(0, 500));

  const validCols = await getValidColumns(entity);
  console.log(`[PUT /api/entities/${entity}/${id}] Valid Columns:`, Array.from(validCols).join(", "));
  if (validCols.size === 0) {
    return c.json({ error: "Entity not found" }, 404);
  }

  // Filter body to only include valid columns and exclude auto-generated ones
  const filteredBody: Record<string, any> = {};
  for (const [k, v] of Object.entries(body)) {
    if (validCols.has(k) && k !== 'id' && k !== 'created_at' && k !== 'updated_at' && k !== 'client_id') {
      filteredBody[k] = v;
    }
  }

  if (Object.keys(filteredBody).length === 0) {
    // If nothing to update, just return the existing record
    try {
      const result = await client.queryObject(`SELECT * FROM "${entity}" WHERE id = $1`, [id]);
      return c.json(result.rows[0] || {});
    } catch (e) {
      return c.json({ error: "No data provided" }, 400);
    }
  }

  const setClauses = Object.keys(filteredBody).map((k, i) => `"${k}" = $${i + 2}`).join(", ");
  const values = [id, ...Object.values(filteredBody)];
  
  let query = `UPDATE "${entity}" SET ${setClauses} WHERE id = $1`;

  // MULTI-TENANCY ENFORCEMENT
  if (validCols.has("client_id")) {
    query += ` AND client_id = $${values.length + 1}`;
    values.push(user.client_id);
  }

  query += ` RETURNING *`;

  try {
    const result = await client.queryObject(query, values);
    if (result.rows.length === 0) {
      return c.json({ error: "Not found" }, 404);
    }
    return c.json(result.rows[0]);
  } catch (error: any) {
    console.error("PUT Error:", error);
    return c.json({ error: error.message }, 500);
  }
});

// DELETE /api/entities/:entity/:id - delete
entityRouter.delete("/:entity/:id", async (c) => {
  const entity = sanitizeTableName(c.req.param("entity"));
  const id = c.req.param("id");
  const user = c.get("jwtPayload") as any;

  const validCols = await getValidColumns(entity);

  try {
    let query = `DELETE FROM "${entity}" WHERE id = $1`;
    const args: any[] = [id];

    // MULTI-TENANCY ENFORCEMENT
    if (validCols.has("client_id")) {
      query += ` AND client_id = $2`;
      args.push(user.client_id);
    }

    query += ` RETURNING id`;

    const result = await client.queryObject(query, args);
    if (result.rows.length === 0) {
      return c.json({ error: "Not found" }, 404);
    }
    return c.json({ success: true, deletedId: id });
  } catch (error: any) {
    return c.json({ error: error.message }, 500);
  }
});
