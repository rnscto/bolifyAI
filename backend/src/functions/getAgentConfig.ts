...
export default async function getAgentConfig(c: any) {
  const req = c.req.raw || c.req;
  try {
    const appId = Deno.env.get('BASE44_APP_ID');
    /* const base44 = ... */;

    const { call_sid, stream_sid, agent_id } = await c.req.json();

    if (!call_sid && !stream_sid && !agent_id) {
      return c.json({ data: { error: 'call_sid, stream_sid, or agent_id required' } }, 400);
    }
