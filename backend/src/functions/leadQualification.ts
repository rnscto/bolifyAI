...
export default async function leadQualification(c: any) {
  ...
  try {
    ...
    if (!event || event.entity_name !== 'Lead' || event.type !== 'update') {
      return c.json({ data: { success: true, skipped: 'not_lead_update' } });
    }
    ...
    if (!scoreChanged && !sentimentChanged) {
      return c.json({ data: { success: true, skipped: 'no_score_change' } });
    }
    ...
    // Mirror the tier change into the Postgres leads mirror (best-effort).
    base44.functions.invoke('pgLeadSync', {
      lead: { ...lead, id: leadId, qualification_tier: tier }
    }).catch(() => {});
