...
function formatOrder(order: any) {
  ...
  })).filter((t: any) => t.tracking_number);
...
export default async function shopifyLookup(c: any) {
  try {
    const { client_id, lookup_type, query } = await c.req.json().catch(() => ({}));
    ...
    const integrations = await base44.entities.MarketplaceIntegration.filter({
      client_id,
      platform: 'shopify',
      status: 'active'
    });
    ...
    if (lookup_type === 'order_by_number') {
      ...
      if (!res.ok) {
        ...
        return c.json({ data: { success: false, error: `Shopify API error: \${res.status}` } });
      }
    ...
    // ─── Orders by customer phone ───
      ...
      const phoneOrders = (data.orders || []).filter((o: any) => {
    ...
    // ─── Orders by customer email ───
      ...
      if (!custRes.ok) {
        return c.json({ data: { success: false, error: `Shopify customer search error: \${custRes.status}` } });
      }
    ...
    // ─── Product availability ───
      ...
      if (!res.ok) {
        return c.json({ data: { success: false, error: `Shopify product search error: \${res.status}` } });
      }
      const data = await res.json();
      const products = (data.products || []).map((p: any) => ({
        ...
        total_inventory: (p.variants || []).reduce((sum: number, v: any) => sum + (v.inventory_quantity || 0), 0),
        available: (p.variants || []).some((v: any) => (v.inventory_quantity || 0) > 0)
      }));
    ...
    // ─── Refund status for an order ───
      ...
      if (!res.ok) {
        return c.json({ data: { success: false, error: `Shopify refund lookup error: \${res.status}` } });
      }
      const data = await res.json();
      const refunds = (data.refunds || []).map((r: any) => ({
        id: r.id,
        created_at: r.created_at,
        note: r.note,
        total_amount: r.transactions?.reduce((s: number, t: any) => s + parseFloat(t.amount || 0), 0) || 0,
        currency: r.currency,
        refund_line_items: (r.refund_line_items || []).map((li: any) => ({
          quantity: li.quantity,
    ...
    // ─── Shipping/tracking for an order ───
      ...
      if (!res.ok) {
        return c.json({ data: { success: false, error: `Shopify fulfillment lookup error: \${res.status}` } });
      }
      const data = await res.json();
