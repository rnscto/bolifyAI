...
export default async function shopifyLookup(c: any) {
  ...
  try {
    ...
    // In test_mode, accept any status; otherwise require active
    ...
    if (integrations.length === 0) {
      return c.json({ data: {
        success: false,
        error: test_mode
          ? 'No Shopify integration found for this client. Please save your credentials first.'
          : 'No active Shopify integration found for this client'
      } });
    }
