...
export default async function generatePartnerInvoice(c: any) {
  ...
  try {
    ...
    const { payout_id } = await c.req.json();
    if (!payout_id) return c.json({ data: { error: 'payout_id required' } }, 400);

    // Get payout and partner data
    const payouts = await base44.asServiceRole.entities.PartnerPayout.filter({ id: payout_id });
    if (payouts.length === 0) return c.json({ data: { error: 'Payout not found' } }, 404);
    const payout = payouts[0];

    const partners = await base44.asServiceRole.entities.Partner.filter({ id: payout.partner_id });
    if (partners.length === 0) return c.json({ data: { error: 'Partner not found' } }, 404);
