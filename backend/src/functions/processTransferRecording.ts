...
export default async function processTransferRecording(c: any) {
  ...
  try {
    ...
    if (!callLog.transferred_to) {
      return c.json({ data: { skipped: true, reason: 'Not a transferred call' } });
    }
    ...
    if (!callLog.recording_url) {
      ...
      return c.json({ data: { skipped: true, reason: 'No recording_url available yet' } });
    }
    ...
    if (!audioResponse.ok) {
      ...
      if (!audioResponse.ok) {
        ...
        return c.json({ data: { error: 'Failed to download recording', status: audioResponse.status } }, 500);
      }
    }
    ...
    if (audioBuffer.byteLength < 1000) {
      ...
      return c.json({ data: { skipped: true, reason: 'Recording file too small' } });
    }
    ...
    if (!sttResponse.ok) {
      ...
      return c.json({ data: { error: 'Transcription failed', detail: errText } }, 500);
    }
    ...
    if (fullTranscript.length < 30) {
      ...
      return c.json({ data: { skipped: true, reason: 'Transcript too short' } });
    }
