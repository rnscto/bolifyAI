...
//
// Twilio Media Streams protocol (vs Smartflo):
//   • Audio format: mu-law 8kHz base64 (SAME as Smartflo — no resample diff)
//   • Frame size: 160 bytes / 20ms (SAME)
//   • Events: 'connected', 'start', 'media', 'stop', 'mark'
//   • start.streamSid / start.callSid same shape as Smartflo
//   • Custom Parameters from <Parameter> in TwiML arrive as start.customParameters
//
// Brain: AZURE GPT-Realtime GA (NOT Gemini), using:
//   AZURE_REALTIME_GA=true semantics from streamRealtimeOutgoing.
// ═══════════════════════════════════════════════════════════════════════════

// ─── Audio (mu-law 8k ↔ PCM16 24k) — identical to streamRealtimeOutgoing ───
...
// ─── Hallucinated-script filter (Whisper foreign-script guard) ───
...
async function saveCallRecord(session, reqId, duration) {
  ...
  try {
    ...
    console.log(`[${reqId}] 💾 Saved (twilio): ${session.callLogId}, score=${leadScore}`);
    ...
    // Twilio webhook already schedules fetchTwilioRecording — no need to dupe here
...
// human warmth, and gender-by-voice addressing so quality no longer depends
// on each client's prompt wording.
// ═══════════════════════════════════════════════════════════════════════
...
// ═══ MAIN ═══
export default async function streamTwilioOutgoing(c: any) {
  const req = c.req.raw || c.req;
  const reqId = Math.random().toString(36).substring(2, 10);
  const isWS = (req.headers.get('upgrade')||'').toLowerCase()==='websocket';
  console.log(`[${reqId}] 📨 ${req.method} (twilio-outgoing), ws=${isWS}`);
  if(!isWS){
    const host = req.headers.get('host')||'localhost';
    return c.json({ data: { success:true, wss_url:`wss://${host}/functions/streamTwilioOutgoing`, flow:'twilio-outgoing-realtime-ga' } }, 200);
  }

  let twilioSocket, response;
  try { const u=Deno.upgradeWebSocket(req); twilioSocket=u.socket; response=u.response; }
  ...
  async function executeToolCall(callId, name, argsStr){
    ...
    try {
      ...
      if(name==='search_knowledge_base'){
        ...
        const goodbyeRegex = /(bye|goodbye|alvida|namaste|namaskar|dhanyav[aā]d|thank\s*you|thanks|shukriya|theek\s*hai\s*bye|ok\s*bye|fir\s*milte|chalo\s*bye|have a good|take care|cheers|see you|बाय|अलविदा|धन्यवाद|शुक्रिया|नमस्ते|नमस्कार|फिर मिलते)/i;
        if (!goodbyeRegex.test(recentCustomer)) {
          ...
          sendToRealtime({type:'conversation.item.create', item:{type:'function_call_output', call_id:callId, output:JSON.stringify({error:'Customer has NOT said goodbye yet. Continue the conversation. Do NOT call end_call until the customer explicitly says bye/thank you/namaste/dhanyavaad. Ask your next question.'})}});
          sendToRealtime({type:'response.create'});
          return;
        }
        result={success:true};
        ...
        setTimeout(()=>{
          ...
          saveCallRecord(session, reqId, d).then(()=>{ if(twilioSocket.readyState===WebSocket.OPEN) twilioSocket.close(); });
        }, 1500);
        return;
      }
  ...
  function connectRealtime(){
    // GA only — this is the Twilio brain, locked to gpt-realtime-2 GA endpoint.
  ...
  function handleRealtimeMessage(msg){
    ...
    if((t==='response.output_audio.delta' || t==='response.audio.delta') && msg.delta){
      ...
      if(twilioSocket.readyState===WebSocket.OPEN && session.streamSid) sendMulawToTwilio(m);
      return;
    }
    ...
    if(t==='input_audio_buffer.speech_started'){
      // Twilio: send 'clear' to flush queued playback
      if(twilioSocket.readyState===WebSocket.OPEN && session.streamSid){
  ...
  function sendMulawToTwilio(b){
    // Twilio expects mu-law base64 in 160-byte frames (20ms). Same as Smartflo.
    const C = 960;
    for(let i=0;i<b.length;i+=C){
      ...
      twilioSocket.send(JSON.stringify({event:'media', streamSid:session.streamSid, media:{payload:uint8ToBase64(chunk)}}));
    }
  }
  ...
  twilioSocket.onmessage = async (event) => {
    try {
      ...
      if(msg.event === 'start'){
        ...
        // Twilio passes <Parameter> values inside start.customParameters
        ...
        console.log(`[${reqId}] 📞 START twilio outbound: streamSid=${session.streamSid}, call_log_id=${callLogId}`);
        if(callLogId){
  ...
  twilioSocket.onclose = async () => {
    session._callEnded = true;
    const d = Math.round((Date.now() - session.startTime) / 1000);
    if(session.realtimeWs?.readyState === WebSocket.OPEN) session.realtimeWs.close();
    if(session.callLogId) await saveCallRecord(session, reqId, d);
  };
  twilioSocket.onerror = () => { if(session.realtimeWs?.readyState === WebSocket.OPEN) session.realtimeWs.close(); };
