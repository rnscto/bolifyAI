...
export default async function crmAutomation(c: any) {
  ...
  try {
    ...
    if (!event || !data) {
      return c.json({ data: { success: false, error: 'Invalid payload' } }, 400);
    }
    ...
    if (entityName === 'Lead') {
      ...
      if (isScoreOnlyChange) {
        ...
        return c.json({ data: { success: true, skipped: 'score_only_update' } });
      }
      ...
      if (eventType === 'update' && data.status === 'interested' && old_data?.status !== 'interested') {
        if (data.client_id) {
          ...
          if (configs.length === 0) {
            ...
            return c.json({ data: { success: true, skipped: 'no_crm_config' } });
          }
