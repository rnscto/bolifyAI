...
export default async function crmFollowupCheck(c: any) {
  ...
  try {
    ...
    // Scheduled automation — use service role from request
    const client = base44;;
    ...
    return c.json({ data: { success: true, created: createdCount } });
  } catch (error) {
