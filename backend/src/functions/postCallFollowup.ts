...
// Routes through sendViaClientProvider so the lead sees the client's own
// sender (e.g. noreply@analyticslearners.com) instead of DoNotReply@vaaniai.io.
// sendViaClientProvider falls back to Vaani ACS only when the client has no
// provider configured (e.g. platform retention emails).
async function sendLeadEmail(svc, { to, fromName, subject, html, clientId }) {
  const res = await svc.functions.invoke('sendViaClientProvider', {
...
export default async function postCallFollowup(c: any) {
  ...
  try {
    ...
    if (event) {
      ...
      if (data.status !== 'completed' || old_data?.status === 'completed') {
        return c.json({ data: { success: true, skipped: 'not_newly_completed' } });
      }
      if (!data.conversation_summary && !data.transcript) {
        return c.json({ data: { success: true, skipped: 'no_transcript_yet' } });
      }
    } else if (payload.call_log_id) {
      // Direct invocation
      callLog = await svc.entities.CallLog.get(payload.call_log_id);
    } else {
      return c.json({ data: { error: 'Invalid payload' } }, 400);
    }
    ...
    if (isCampaignCall) {
      ...
      return c.json({ data: { success: true, skipped: 'campaign_call' } });
    }
    ...
    if (!clientId || clientId === 'unknown') {
      ...
      return c.json({ data: { success: true, skipped: 'unknown_client' } });
    }

    // ===== LOAD CONTEXT =====
    const client = await svc.entities.Client.get(clientId);
    if (!client) {
      return c.json({ data: { success: true, skipped: 'client_not_found' } });
    }
  ...
  <div style="padding: 30px; background: white; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 12px 12px;">
    ...
    <p style="color: #94a3b8; font-size: 12px; text-align: center;">
      Sent by ${client.company_name} powered by VaaniAI
        ...
        await sendLeadEmail(svc, { to: lead.email, fromName: client.company_name, subject: aiContent.subject, html: emailHtml, clientId });
