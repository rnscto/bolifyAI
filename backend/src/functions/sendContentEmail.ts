...
export default async function sendContentEmail(c: any) {
  ...
  try {
    ...
    if (!leadId || !clientId) {
      return c.json({ data: { error: 'lead_id and client_id are required' } }, 400);
    }
    ...
    if (!recipientEmail) {
      return c.json({ data: { success: false, skipped: 'no_email', lead_name: lead?.name } });
    }
    ...
    if (!callLog) {
      return c.json({ data: { success: false, skipped: 'no_completed_call' } });
    }
