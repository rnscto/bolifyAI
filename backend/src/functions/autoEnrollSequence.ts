...
export default async function autoEnrollSequence(c: any) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      // Process ONE tier per run (rotated by clock) to keep each cron run light
      // and avoid gateway/entity rate-limits. 4 tiers → full cycle every 4 runs.
      const tiers = ['hot', 'warm', 'nurture', 'cold'];
      const tier = tiers[Math.floor(Date.now() / 60000) % tiers.length];
      console.log(`[autoEnrollSequence] This run handles tier: ${tier}`);

      for (const _t of [tier]) {
        const leads = await base44.entities.Lead.filter({ qualification_tier: tier }, '-updated_date', 30);
        for (const lead of leads) {
          ...
          // Skip if already enrolled in an active sequence (single query — the
          // active check alone prevents duplicate enrollment).
      ...
      return c.json({ data: { success: true, mode: 'batch', ...results } });
    }
    ...
    if (!lead_id || !client_id || !qualification_tier) {
      return c.json({ data: { error: 'Missing lead_id, client_id, or qualification_tier' } }, 400);
    }

    // Skip disqualified / do_not_call
    if (qualification_tier === 'disqualified' || call_outcome === 'do_not_call') {
      return c.json({ data: { success: true, skipped: 'disqualified' } });
    }

    // Get lead
    const lead = await base44.entities.Lead.get(lead_id);
    if (!lead?.email) {
      return c.json({ data: { success: true, skipped: 'no_email' } });
    }
    ...
    if (existingEnrollments.length > 0) {
      ...
      return c.json({ data: { success: true, skipped: 'already_enrolled', sequence_id: existingEnrollments[0].sequence_id } });
    }
    ...
    if (!sequence || !sequence.steps || sequence.steps.length === 0) {
      return c.json({ data: { success: true, skipped: 'no_valid_sequence' } });
    }
