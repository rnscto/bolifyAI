...
export default async function dailyTaskDigest(c: any) {
  ...
  try {
    ...
    if (allPending.length === 0) {
      ...
      return c.json({ data: { success: true, emails_sent: 0, reason: 'no_pending_tasks' } });
    }
    ...
    return c.json({ data: { success: true, emails_sent: emailsSent, clients_processed: Object.keys(byClient).length } });
  } catch (error) {
