...
export default function AgreementViewer({ partnerId }) {
    ...
    return (
      <Card>
        <CardContent className="py-8 text-center text-gray-500">
          ...
          <p className="text-sm mt-1">Agreements will appear here once generated.</p>
        </CardContent>
      </Card>
    );
  }
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                  agr.status === 'signed' ? 'bg-green-100' : 'bg-yellow-100'
                }`}>
                  {agr.status === 'signed' ? (
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {agr.status === 'signed'
                      ? `Signed on ${moment(agr.signed_date).format('DD MMM YYYY, h:mm A')}`
                      : 'Awaiting signature'}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                {agr.status === 'signed' && agr.rendered_html && (
                  <>
                    <Button size="sm" variant="outline" onClick={() => setViewHtml(agr.rendered_html)}>
                      <FileText className="w-4 h-4 mr-1" /> View
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => handlePrint(agr)}>
            ...
            {agr.status === 'signed' && (
              <div className="mt-3 pt-3 border-t text-xs text-gray-400 flex gap-4 flex-wrap">
                ...
                <span>Template v{agr.template_version}</span>
              </div>
            )}
          </CardContent>
        </Card>
