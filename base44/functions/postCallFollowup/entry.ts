...
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
  ...
  <div style="padding: 30px; background: white; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 12px 12px;">
    ...
    <p style="color: #94a3b8; font-size: 12px; text-align: center;">
      Sent by ${client.company_name} powered by Bolify AI
    </p>
  </div>
</div>`;
...
Keep it personal, mention key point from the call, include CTA. No links.`,
    ...
    if (isExpiredClient || isRetentionCall) {
      ...
      const retentionEmail = await azureLLM(
        `You are Bolify AI's retention email specialist. Write a personalized retention email for a platform client who hasn't subscribed.
...
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
  <div style="background: linear-gradient(135deg, #1a365d, #2d3748); padding: 24px 30px; border-radius: 12px 12px 0 0; text-align: center;">
    <h2 style="color: white; margin: 0;">Bolify AI</h2>
  ...
  <div style="padding: 20px 30px; background: #f7fafc; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 12px 12px; text-align: center;">
    <a href="https://bolify.ai" style="display: inline-block; background: linear-gradient(135deg, #e67e22, #d35400); color: white; padding: 14px 40px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 16px;">Subscribe Now — ₹6,500/mo</a>
      ...
      if (client.phone) {
        try {
          const retentionRCS = await azureLLM(
            `Write a short retention RCS/SMS (max 160 chars) for ${client.company_name}. Account expired. ${retentionConfig.active_offer ? 'Offer: ' + retentionConfig.active_offer : ''} Mention Bolify AI. Include urgency.`,
            'You are an SMS copywriter. Always respond in valid JSON.',
            { type: "object", properties: { message: { type: "string" } } }
          );
