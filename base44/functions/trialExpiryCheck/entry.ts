...
async function sendEmail({ to, subject, html, displayName }) {
  ...
  await client.sendAsync({ from: `${displayName || 'Bolify AI'} <${smtpFrom}>`, to, subject, attachment: [{ data: html, alternative: true }] });
  ...
  try {
    ...
    for (const client of trialClients) {
      ...
      if (daysLeft <= 0) {
        ...
        await sendEmail({
          to: client.email,
          subject: 'Your Bolify AI trial has expired — special offer inside!',
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <div style="padding: 30px; background: white; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 12px 12px;">
                ...
                <div style="text-align: center; margin: 30px 0;">
                  <a href="https://bolify.ai" style="background: linear-gradient(135deg, #e67e22, #f39c12); color: white; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 16px;">Subscribe Now — ₹6,500/mo</a>
                </div>
      ...
      if (daysLeft === 3) {
        await sendEmail({
          to: client.email,
          subject: `Only 3 days left on your Bolify AI trial!`,
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <div style="background: linear-gradient(135deg, #1a365d, #2d3748); padding: 30px; text-align: center; border-radius: 12px 12px 0 0;">
                <h1 style="color: white; margin: 0;">Bolify AI</h1>
              ...
              <div style="padding: 30px; background: white; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 12px 12px;">
                ...
                <div style="text-align: center; margin: 30px 0;">
                  <a href="https://bolify.ai" style="background: linear-gradient(135deg, #e67e22, #f39c12); color: white; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 16px;">View Subscription Plans</a>
                </div>
              </div>
            </div>
          `
      ...
      if (daysLeft === 1) {
        await sendEmail({
          to: client.email,
          subject: `⚠️ Last day of your Bolify AI trial!`,
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              ...
              <div style="padding: 30px; background: white; border: 1px solid #e2e8f0; border-top: none; border-radius: 0 0 12px 12px;">
                ...
                <div style="text-align: center; margin: 30px 0;">
                  <a href="https://bolify.ai" style="background: #c53030; color: white; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 16px;">Subscribe Now</a>
                </div>
