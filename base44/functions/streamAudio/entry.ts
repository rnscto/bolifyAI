...
// The Smartflo JWT is valid platform-wide. Persist it in the single SmartfloAuth row so every
// function/isolate reuses one token instead of each logging in separately (logins are what
// trigger Smartflo lockouts). All entity access is wrapped — any failure falls back to login.
